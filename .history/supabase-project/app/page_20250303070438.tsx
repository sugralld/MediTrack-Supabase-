...
export default function Home() {
  ...
  const handleGuestLogin = async () => {
    ...
    console.log("Guest login sukses!", data);
    router.refresh();
  };
  ...
  const getUserRole = async () => {
    ...
    if (error) {
      console.error("Error ambil role:", error.message);
      return null;
    }
  ...
  useEffect(() => {
    const checkLogin = async () => {
      ...
      if (!session) {
        console.log("Belum login, jalankan login guest");
      ...
      setCheckingSession(false); // supaya state tahu udah kelar cek
    };
  ...
  if (checkingSession) {
    return <div>Loading...</div>; // biar ga ngeblank/ngestuck
  }
