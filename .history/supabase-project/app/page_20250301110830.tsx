...
export default function Home() {
  ...
  const [filteredInterpretations, setFilteredInterpretations] = useState<IInterpretation[]>([]);
  ...
  useEffect(() => {
    const fetchMedicines = async () => {
      try {
        ...
        data.sort((a: IInterpretation, b: IInterpretation) => a.name.localeCompare(b.name));
  ...
  const handleFilter = (category: string) => {
    setFilteredInterpretations(
      category === "Semua"
        ? interpretations
        : interpretations.filter((item) => item.category === category)
    );
  };
          ...
          <Grid container spacing={3}>
            {filteredInterpretations.map((medicine) => (
              <Grid item xs={12} sm={6} md={4} key={medicine.id}>
                <ProductCard medId={medicine.id} {...medicine} />
              </Grid>
            ))}
          </Grid>
        </Suspense>
